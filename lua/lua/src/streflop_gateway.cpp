...
extern "C" {
	...
	lua_Number streflop_sinh(lua_Number x) {
			return streflop::sinh(x);
	}

	lua_Number streflop_cosh(lua_Number x) {
			return streflop::cosh(x);
	}

	lua_Number streflop_tanh(lua_Number x) {
			return streflop::tanh(x);
	}
...
//FIXME	lua_Number streflop_modf(lua_Number x, lua_Number* ptr) {
//			return streflop::modf(x, ptr);
//	}
	...
	lua_Number streflop_random() {
			// respect the original lua code that uses rand, and rand
			// is auto-seeded always with the same value
