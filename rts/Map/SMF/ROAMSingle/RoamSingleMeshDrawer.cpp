...
#define LOG_SECTION_ROAM "RoamSingleThreadedMeshDrawer"
...
CRoamSingleMeshDrawer::CRoamSingleMeshDrawer(CSMFGroundDrawer* gd)
	: CEventClient("[CRoamSingleMeshDrawer]", 278546, false)
	, smfGroundDrawer(gd)
	, lastGroundDetail{0, 0}
{
	...
	PatchSingle::SwitchRenderMode(configHandler->GetInt("ROAM"));
...
void CRoamSingleMeshDrawer::DrawInMiniMap()
{
	...
	for (const PatchSingle& p: patchMeshGrid[MESH_NORMAL]) {
		...
		glRectf(p.coors.x, p.coors.y, p.coors.x + PATCH_SIZE_SINGLE, p.coors.y + PATCH_SIZE_SINGLE);
	}
