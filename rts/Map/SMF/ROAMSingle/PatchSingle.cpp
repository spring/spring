...
static std::vector<CTriNodePoolSingle> pools[CRoamSingleMeshDrawer::MESH_COUNT];
...
void CTriNodePoolSingle::ResetAll(bool shadowPass)
{
	...
	for (CTriNodePoolSingle& pool: pools[shadowPass]) {
...
void PatchSingle::RecursGenIndices(const TriTreeNodeSingle* tri, const int2 left, const int2 right, const int2 apex)
{
	if (tri->IsLeaf()) {
...
void PatchSingle::GenerateIndices()
{
	...
	RecursGenIndices(&baseLeft,  int2(         0, PATCH_SIZE_SINGLE), int2(PATCH_SIZE_SINGLE,          0), int2(         0,          0));
	RecursGenIndices(&baseRight, int2(PATCH_SIZE_SINGLE,          0), int2(         0, PATCH_SIZE_SINGLE), int2(PATCH_SIZE_SINGLE, PATCH_SIZE_SINGLE));
}



void PatchSingle::GenerateBorderVertices()
{
	if (!isTesselated)
		return;
	...
	borderVertices.reserve((PATCH_SIZE_SINGLE + 1) * 2);
...
void PatchSingle::RecursGenBorderVertices(
...
{
	const int vIdx = (pos.y * (PATCH_SIZE_SINGLE + 1) + pos.x) * 3 + 1;
...
void PatchSingle::UploadVertices()
{
	glBindVertexArray(vertexArrays[0]);
...
void PatchSingle::UploadBorderVertices()
{
	glBindVertexArray(vertexArrays[1]);
...
void PatchSingle::UploadIndices()
{
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexBuffer);
...
public:
	...
	void ResetState(CCamera* c, PatchSingle* p, int npx, int npy) {
		testCamera = c;
		patchArray = p;
		numPatchesX = npx;
		numPatchesY = npy;
