...
#include "PatchSingle.h"
#include "RoamSingleMeshDrawer.h"
...
PatchSingle::RenderMode PatchSingle::renderMode = PatchSingle::VBO;

static size_t CUR_POOL_SIZE_SINGLE =                 0; // split over all threads
static size_t MAX_POOL_SIZE_SINGLE = NEW_POOL_SIZE_SINGLE * 8; // upper limit for ResetAll


static std::vector<CTriNodePoolSingle> poolsSingle[CRoamSingleMeshDrawer::MESH_COUNT];


void CTriNodePoolSingle::InitPools(bool shadowPass, size_t newPoolSize)
{
	const int numThreads = ThreadPool::GetMaxThreads();
	const size_t thrPoolSize = std::max((CUR_POOL_SIZE_SINGLE = newPoolSize) / numThreads, newPoolSize / 3);

	try {
		poolsSingle[shadowPass].clear();
		poolsSingle[shadowPass].reserve(numThreads);

		for (int i = 0; i < numThreads; i++) {
			poolsSingle[shadowPass].emplace_back(thrPoolSize + (thrPoolSize & 1));
		}
		...
		InitPools(shadowPass, MAX_POOL_SIZE_SINGLE = (newPoolSize - (newPoolSize >> 2)));
	}
}

void CTriNodePoolSingle::ResetAll(bool shadowPass)
{
	bool outOfNodes = false;

	for (CTriNodePoolSingle& pool: poolsSingle[shadowPass]) {
	...
	if (!outOfNodes)
		return;
	if (CUR_POOL_SIZE_SINGLE >= MAX_POOL_SIZE_SINGLE)
		return;

	InitPools(shadowPass, std::min<size_t>(CUR_POOL_SIZE_SINGLE * 2, MAX_POOL_SIZE_SINGLE));
}


CTriNodePoolSingle* CTriNodePoolSingle::GetPool(bool shadowPass)
{
	return &(poolsSingle[shadowPass][ThreadPool::GetThreadNum()]);
}


CTriNodePoolSingle::CTriNodePoolSingle(const size_t poolSize): nextTriNodeIdx(0)
{
...
void CTriNodePoolSingle::Reset()
{
	// reinit all entries; faster than calling TriTreeNodeSingle's ctor
	if (nextTriNodeIdx > 0)
		memset(&pool[0], 0, sizeof(TriTreeNodeSingle) * nextTriNodeIdx);

	nextTriNodeIdx = 0;
}

bool CTriNodePoolSingle::Allocate(TriTreeNodeSingle*& left, TriTreeNodeSingle*& right)
{
...
PatchSingle::PatchSingle()
	...
	, vertexIndexBuffer(0)
{
	varianceLeft.resize(1 << VARIANCE_DEPTH_SINGLE);
	varianceRight.resize(1 << VARIANCE_DEPTH_SINGLE);
}
...
void PatchSingle::Init(CSMFGroundDrawer* _drawer, int patchX, int patchZ)
{
	coors.x = patchX;
	coors.y = patchZ;
	...
	vertices.resize(3 * (PATCH_SIZE_SINGLE + 1) * (PATCH_SIZE_SINGLE + 1));
	unsigned int index = 0;

	// initialize vertices
	for (int z = coors.y; z <= (coors.y + PATCH_SIZE_SINGLE); z++) {
		for (int x = coors.x; x <= (coors.x + PATCH_SIZE_SINGLE); x++) {
...
void PatchSingle::Reset()
{
	// reset the important relationships
	baseLeft  = TriTreeNodeSingle();
	baseRight = TriTreeNodeSingle();
...
void PatchSingle::UpdateHeightMap(const SRectangle& rect)
{
	const float* hMap = readMap->GetCornerHeightMapUnsynced();

	for (int z = rect.z1; z <= rect.z2; z++) {
		for (int x = rect.x1; x <= rect.x2; x++) {
			const int vindex = (z * (PATCH_SIZE_SINGLE + 1) + x) * 3;
...
void PatchSingle::VBOUploadVertices()
{
	if (renderMode == VBO) {
		// Upload vertexBuffer
...
//
bool PatchSingle::Split(TriTreeNodeSingle* tri)
{
...
//
void PatchSingle::RecursTessellate(TriTreeNodeSingle* tri, const int2 left, const int2 right, const int2 apex, const int node)
{
	...
	if (node < (1 << VARIANCE_DEPTH_SINGLE)) {
		...
		triVariance = (std::min(currentVariance[node], varianceMaxLimit) * PATCH_SIZE_SINGLE * size) * camDistLODFactor;
	}
...
void PatchSingle::RecursRender(const TriTreeNodeSingle* tri, const int2 left, const int2 right, const int2 apex)
{
	if (tri->IsLeaf()) {
		indices.push_back(apex.x  + apex.y  * (PATCH_SIZE_SINGLE + 1));
		indices.push_back(left.x  + left.y  * (PATCH_SIZE_SINGLE + 1));
		indices.push_back(right.x + right.y * (PATCH_SIZE_SINGLE + 1));
		return;
	}
...
void PatchSingle::GenerateIndices()
{
	indices.clear();
	RecursRender(&baseLeft,  int2(         0, PATCH_SIZE_SINGLE), int2(PATCH_SIZE_SINGLE,          0), int2(         0,          0));
	RecursRender(&baseRight, int2(PATCH_SIZE_SINGLE,          0), int2(         0, PATCH_SIZE_SINGLE), int2(PATCH_SIZE_SINGLE, PATCH_SIZE_SINGLE));
}

float PatchSingle::GetHeight(int2 pos)
{
	const int vindex = (pos.y * (PATCH_SIZE_SINGLE + 1) + pos.x) * 3 + 1;
...
//
float PatchSingle::RecursComputeVariance(
	const   int2 left,
	const   int2 rght,
	const   int2 apex,
	const float3 hgts,
...
//
void PatchSingle::ComputeVariance()
{
	{
		currentVariance = &varianceLeft[0];

		const   int2 left = {         0, PATCH_SIZE_SINGLE};
		const   int2 rght = {PATCH_SIZE_SINGLE,          0};
		const   int2 apex = {         0,          0};
		const float3 hgts = {
			GetHeight(left),
			GetHeight(rght),
...

	{
		...
		const   int2 left = {PATCH_SIZE_SINGLE,          0};
		const   int2 rght = {         0, PATCH_SIZE_SINGLE};
		const   int2 apex = {PATCH_SIZE_SINGLE, PATCH_SIZE_SINGLE};
		const float3 hgts = {
			GetHeight(left),
			GetHeight(rght),
			GetHeight(apex),
...
//
bool PatchSingle::Tessellate(const float3& camPos, int viewRadius, bool shadowPass)
{
	// Set/Update LOD params (FIXME: wrong height?)
	float3 midPos;
	midPos.x = (coors.x + PATCH_SIZE_SINGLE / 2) * SQUARE_SIZE;
	midPos.z = (coors.y + PATCH_SIZE_SINGLE / 2) * SQUARE_SIZE;
	...
	curTriPool = CTriNodePoolSingle::GetPool(shadowPass);
...

	{
		...
		const int2 left = {coors.x,              coors.y + PATCH_SIZE_SINGLE};
		const int2 rght = {coors.x + PATCH_SIZE_SINGLE, coors.y             };
	...
	}
	{
		...
		const int2 left = {coors.x + PATCH_SIZE_SINGLE, coors.y             };
		const int2 rght = {coors.x,              coors.y + PATCH_SIZE_SINGLE};
		const int2 apex = {coors.x + PATCH_SIZE_SINGLE, coors.y + PATCH_SIZE_SINGLE};
...
void PatchSingle::RecursBorderRender(
	CVertexArray* va,
	...
	if (tri->IsLeaf()) {
		const float3& v1 = *(float3*)&vertices[(apex.x + apex.y * (PATCH_SIZE_SINGLE + 1))*3];
		const float3& v2 = *(float3*)&vertices[(left.x + left.y * (PATCH_SIZE_SINGLE + 1))*3];
		const float3& v3 = *(float3*)&vertices[(rght.x + rght.y * (PATCH_SIZE_SINGLE + 1))*3];
...
void PatchSingle::GenerateBorderIndices(CVertexArray* va)
{
	va->Initialize();

	#define PS PATCH_SIZE_SINGLE
...
void PatchSingle::SetSquareTexture() const
{
	smfGroundDrawer->SetupBigSquare(coors.x / PATCH_SIZE_SINGLE, coors.y / PATCH_SIZE_SINGLE);
}


void PatchSingle::SwitchRenderMode(int mode)
{
	if (mode < 0) {
		mode = renderMode + 1;
		mode %= 3;
	...
	CRoamSingleMeshDrawer::ForceTesselation();
}
...
#if 0
void PatchSingle::UpdateVisibility(CCamera* cam)
{
	const float3 mins( coors.x               * SQUARE_SIZE, readMap->GetCurrMinHeight(),  coors.y               * SQUARE_SIZE);
	const float3 maxs((coors.x + PATCH_SIZE_SINGLE) * SQUARE_SIZE, readMap->GetCurrMaxHeight(), (coors.y + PATCH_SIZE_SINGLE) * SQUARE_SIZE);
...
class CPatchInViewCheckerSingle : public CReadMap::IQuadDrawer
{
public:
	void ResetState() {}
	void ResetState(CCamera* c = nullptr, PatchSingle* p = nullptr, int xsize = 0) {
		testCamera = c;
		patchArray = p;
		numPatchesX = xsize;
	}
...
private:
	CCamera* testCamera;
	PatchSingle* patchArray;

	int numPatchesX;
};


void PatchSingle::UpdateVisibility(CCamera* cam, std::vector<PatchSingle>& patches, const int numPatchesX)
{
	#if 0
	// very slow
	for (PatchSingle& p: patches) {
		p.UpdateVisibility(cam);
	}
	#else
	// very fast
	static CPatchInViewCheckerSingle checker;
	...
	readMap->GridVisibility(cam, &checker, 1e9, PATCH_SIZE_SINGLE);
	#endif
}

bool PatchSingle::IsVisible(const CCamera* cam) const {
