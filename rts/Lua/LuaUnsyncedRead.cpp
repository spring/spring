...
int LuaUnsyncedRead::LoadTextVFS(lua_State* L)
{
	const int args = lua_gettop(L); // number of arguments
...
int LuaUnsyncedRead::FileExistsVFS(lua_State* L)
{
	const int args = lua_gettop(L); // number of arguments
	if ((args < 1) || !lua_isstring(L, 1) ||
	    ((args >= 2) && !lua_isstring(L, 2))) {
...
int LuaUnsyncedRead::GetDirListVFS(lua_State* L)
{
	const int args = lua_gettop(L); // number of arguments
	if ((args != 1) || !lua_isstring(L, 1)) {
...
int LuaUnsyncedRead::GetDirList(lua_State* L)
{
	const int args = lua_gettop(L); // number of arguments
	if ((args < 1) || !lua_isstring(L, 1) ||
	    ((args >= 2) && !lua_isstring(L, 2)) ||
