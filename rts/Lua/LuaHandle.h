...
struct luaContextData {
	...
	MatrixData matrixData; // [>0] = stack depth for mode, [0] = matrix mode
	bool listMode; // if creating display list

	MatrixData PushMatrixState() {
		MatrixData md;
		matrixData.swap(md);
		return md;
	}

	MatrixData PushMatrixState(bool lm) {
		listMode = lm;
		return PushMatrixState();
	}

	void PopMatrixState(MatrixData& md) {
		matrixData.swap(md);
	}

	void PopMatrixState(MatrixData& md, bool lm) {
		listMode = lm;
		PopMatrixState(md);
	}

	unsigned int GetMode() {
		MatrixData::iterator i = matrixData.find(0);
		return (i == matrixData.end()) ? GL_MODELVIEW : i->second;
	}

	int GetDepth(unsigned int mode) {
		MatrixData::iterator i = matrixData.find(mode);
	...
	int ApplyMatrixState(MatrixData& m) {
		// validate
	...
	void HandleMatrixError(int error, const char* errsrc) {
		unsigned int mode = GetMode();
		...
		for (MatrixData::iterator i = matrixData.begin(); i != matrixData.end(); ++i) {
			if (i->first == 0)
				continue;
