...
bool CStartPosSelecter::MousePress(int x, int y, int button)
{
	...
	if(InBox(mx,my,readyBox) && gs->Team(gu->myTeam)->startPos.y!=-500){
		...
		net->SendMessage_STARTPOS(gu->myTeam, 1, gs->Team(gu->myTeam)->startPos.x, gs->Team(gu->myTeam)->startPos.y, gs->Team(gu->myTeam)->startPos.z);
		delete this;
		return false;
	}
	...
	net->SendMessage_STARTPOS(gu->myTeam, 0, pos.x, pos.y, pos.z);
