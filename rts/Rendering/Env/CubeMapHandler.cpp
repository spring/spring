...
void CubeMapHandler::CreateReflectionFace(unsigned int glType, const float3& camDir, bool skyOnly)
{
	...
	camera->pos.y = ground->GetHeightAboveWater(camera->pos.x, camera->pos.z) + 50.0f;
	...
	unsigned char* buf)
{
	for (int x = 0; x < size; ++x) {
		...
		const float spec = std::min(1.0f, pow(dot, mapInfo->light.specularExpontent) + pow(dot, 3.0f) * 0.25f);
