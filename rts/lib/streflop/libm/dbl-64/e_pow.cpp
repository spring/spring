...
Double __ieee754_pow(Double x, Double y) {
  ...
  if (x == 0) {
	...
	|| (v.i[HIGH_HALF] & 0x7fffffff) > 0x7ff00000)
      return y;
    if (ABS(y) > 1.0e20) return (y>0)?Double(0.0):INF.x();
    k = checkint(y);
    if (k == -1)
      return y < 0 ? Double(1.0)/x : x;
    else
      return y < 0 ? Double(1.0)/ABS(x) : Double(0.0);                               /* return 0 */
  }
  /* if x<0 */
  if (u.i[HIGH_HALF] < 0) {
    k = checkint(y);
    if (k==0) {
      if ((v.i[HIGH_HALF] & 0x7fffffff) == 0x7ff00000 && v.i[LOW_HALF] == 0) {
	if (x == -1.0) return 1.0;
	else if (x > -1.0) return v.i[HIGH_HALF] < 0 ? INF.x() : Double(0.0);
	else return v.i[HIGH_HALF] < 0 ? Double(0.0) : INF.x();
      }
      else if (u.i[HIGH_HALF] == 0xfff00000 && u.i[LOW_HALF] == 0)
	return y < 0 ? Double(0.0) : INF.x();
  ...
  if (qy > 0x7ff00000 || (qy == 0x7ff00000 && v.i[LOW_HALF] != 0))
    return x == 1.0 ? Double(1.0) : NaNQ.x();
                                                                 /*  if y<2^-0x7fe   */

  if (qx == 0x7ff00000)                              /* x= 2^-0x3ff */
    {if (y == 0) return NaNQ.x();
    return (y>0)?x:Double(0.0); }

  if (qy > 0x45f00000 && qy < 0x7ff00000) {
    if (x == 1.0) return 1.0;
    if (y>0) return (x>1.0)?INF.x():Double(0.0);
    if (y<0) return (x<1.0)?INF.x():Double(0.0);
  }
