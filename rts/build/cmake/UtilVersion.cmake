...
Macro    (ParseSpringVersion varPrefix version)
	CatchRegexGroup("${VERSION_REGEX_ANY}" 1 "${varPrefix}_MAJOR"     "${version}")
	CatchRegexGroup("${VERSION_REGEX_ANY}" 2 "${varPrefix}_PATCH_SET" "${version}")
	CatchRegexGroup("${VERSION_REGEX_DEV}" 3 "${varPrefix}_COMMITS"   "${version}")
	CatchRegexGroup("${VERSION_REGEX_DEV}" 4 "${varPrefix}_HASH"      "${version}")
	CatchRegexGroup("${VERSION_REGEX_DEV}" 5 "${varPrefix}_BRANCH"    "${version}")
...
Macro    (CreateSpringVersionString res_var major patchSet commits hash branch)
...
Macro    (FetchSpringVersion dir prefix)
	...
	If     (EXISTS "${dir}/.git")
		...
		Else   ("${${prefix}_Branch}" STREQUAL "master")
			If     ("${${prefix}_COMMITS}" STREQUAL "")
				Message(AUTHOR_WARNING "Commits since last tag not found in version-string on non-master branch for ${prefix}.")
			EndIf  ("${${prefix}_COMMITS}" STREQUAL "")
		EndIf  ("${${prefix}_Branch}" STREQUAL "master")
	Else   (EXISTS "${dir}/.git")
