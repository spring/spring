...
EXPORT(int) skirmishAiCallback_Game_getGameRulesParams(int skirmishAIId) {
	return CLuaHandleSynced::GetGameParams().size();
}
...
EXPORT(int) skirmishAiCallback_Game_getRulesParamsFloat(int skirmishAIId, const char** keys, const char** values) {
	...
	for (auto& iter : paramsMap) {
		auto& param = params[iter.second];
		if (gameModParamIsVisible(param)) {
			keys[count] = iter.first.c_str();
			values[count] = std::to_string(param.valueInt).c_str();
			count++;
		}
	}
...
EXPORT(int) skirmishAiCallback_Game_getRulesParamsString(int skirmishAIId, const char** keys, const char** values) {
	...
	for (auto& iter : paramsMap) {
		...
		if (gameModParamIsVisible(param)) {
			keys[count] = iter.first.c_str();
			values[count] = param.valueString.c_str();
			count++;
		}
	}
...
EXPORT(int) skirmishAiCallback_Game_getGameRulesParamByName(int skirmishAIId, const char* gameRulesParamName) {
	...
	const LuaRulesParams::HashMap& paramsMap = CLuaHandleSynced::GetGameParamsMap();
	const LuaRulesParams::HashMap::const_iterator pmi = paramsMap.find(gameRulesParamName);
	if (pmi == paramsMap.end()) {
		return res;
	}

	int modParamId = pmi->second;
	const LuaRulesParams::Params& params = CLuaHandleSynced::GetGameParams();
	const LuaRulesParams::Param& param = params[modParamId];
	if (gameModParamIsVisible(param)) {
		res = modParamId;
	}
...
EXPORT(const char*) skirmishAiCallback_GameRulesParam_getName(int skirmishAIId, int gameRulesParamId) {
	...
	const LuaRulesParams::HashMap& paramsMap = CLuaHandleSynced::GetGameParamsMap();
	const LuaRulesParams::Params& params = CLuaHandleSynced::GetGameParams();
	const LuaRulesParams::Param& param = params[gameRulesParamId];
	if (gameModParamIsVisible(param)) {
		std::map<std::string, int>::const_iterator mi, mb, me;
		mb = paramsMap.begin();
		me = paramsMap.end();
		for (mi = mb; mi != me; ++mi) {
			if (mi->second == gameRulesParamId) {
				name = mi->first.c_str();
				break;
			}
		}
...
EXPORT(int) skirmishAiCallback_Unit_getRulesParams(int skirmishAIId, int unitId, const char** params, int params_sizeMax) {
	...
	for (size_t i = 0; i < ps.size(); i++) {
		...
		if (unitModParamIsVisible(p, losMask)) {
...
EXPORT(int) skirmishAiCallback_Unit_getRulesParamsWithNames(int skirmishAIId, int unitId, const char** keys, const char** values) {
	...
	if ((keys == NULL) || (values == NULL)) {
		...
		size_t params_size = 0;
		for (auto& iter : paramsMap) {
			if (unitModParamIsVisible(params[iter.second], losMask)) {
				param_size++;
			}
		}
		return params_size;
...
EXPORT(const char*) skirmishAiCallback_Unit_getRulesParamNameById(int skirmishAIId, int unitId, int rulesParamId) {
	...
	const int losMask = unitModParamLosMask(skirmishAIId, unit);
	if (unitModParamIsVisible(unit->modParams[rulesParamId], losMask)) {
		std::map<std::string, int>::const_iterator mi, mb, me;
...
static void skirmishAiCallback_init(SSkirmishAICallback* callback) {
	...
	callback->GameRulesParam_getName = &skirmishAiCallback_GameRulesParam_getName;
	callback->GameRulesParam_getValueFloat = &skirmishAiCallback_GameRulesParam_getValueFloat;
	callback->GameRulesParam_getValueString = &skirmishAiCallback_GameRulesParam_getValueString;
