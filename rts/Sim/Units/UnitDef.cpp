...
void UnitDef::CreateYardMap(std::string yardMapStr)
{
	...
	// print warnings
	if (idx > yardMap.size())
		LOG_L(L_WARNING, "%s: Given yardmap/blockmap contains %u excess char(s)!", name.c_str(), (unsigned int)(idx - yardMap.size()));

	if (idx > 0 && idx < yardMap.size())
		LOG_L(L_WARNING, "%s: Given yardmap/blockmap requires %u extra char(s)!", name.c_str(), (unsigned int)(yardMap.size() - idx));
