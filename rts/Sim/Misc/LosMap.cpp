...
#define MidpointCircleAlgo(radius, func)                         \
{                                                                \
	int x = radius;                                              \
	int y = 0;                                                   \
...
#define MidpointCircleAlgoPerLine(radius, func)                  \
{                                                                \
	int x_ = radius;                                             \
	int y_ = 0;                                                  \
	int decisionOver2 = 1 - x_;                                  \
	while (x_ >= y_) {                                           \
		CallFunc(func, x_, y_);                                  \
		if (y_ != 0) CallFunc(func, x_, -y_);                    \
                                                                 \
		if (decisionOver2 <= 0) {                                \
			y_++;                                                \
			decisionOver2 += 2 * y_ + 1;                         \
		} else {                                                 \
			if (x_ != y_) {                                      \
				CallFunc(func, y_, x_);                          \
				if (x_ != 0) CallFunc(func, y_, -x_);            \
			}                                                    \
                                                                 \
			y_++;                                                \
			x_--;                                                \
			decisionOver2 += 2 * (y_ - x_) + 1;                  \
		}                                                        \
	}                                                            \
}
...
void CLosMap::AddCircle(SLosInstance* instance, int amount)
{
	...
	MidpointCircleAlgoPerLine(instance->radius,
...
void CLosMap::UnsafeLosAdd(SLosInstance* li) const
{
	...
	MidpointCircleAlgoPerLine(radius,
