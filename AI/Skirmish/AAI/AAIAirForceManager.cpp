...
void AAIAirForceManager::CheckTarget(int unit_id, const UnitDef *def)
{
	...
	if(my_team != ai->Getcb()->GetUnitTeam(unit_id))
	{
		...
		if(x >= 0 && x < ai->Getmap()->xSectors && y >= 0 && y < ai->Getmap()->ySectors)
		{
			...
			if(ai->Getbt()->unitList[def->id-1]->health > 8000)
				max_groups = 3;
			else if(ai->Getbt()->unitList[def->id-1]->health > 4000)
				max_groups = 2;
			else
				max_groups = 1;
...
void AAIAirForceManager::BombBestUnit(float cost, float danger)
{
	...
	for(i = 0; i < cfg->MAX_AIR_TARGETS; ++i)
	{
		if(targets[i].unit_id != -1)
		{
			...
			current = pow(targets[i].cost, cost) / (1.0f + ai->Getmap()->sector[x][y].enemy_stat_combat_power[1] * danger) * targets[i].health / ai->Getbt()->unitList[targets[i].def_id-1]->health ;
