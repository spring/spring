...
namespace CSharpAI
{
    ...
    public class TankController : IUnitRequester, IController
    {
        ...
        public Dictionary<int, IUnitDef> TankDefsById = new Dictionary<int, IUnitDef>();
...
//            if( random.Next( 1, 6 ) < 5 )
   //         {
            if( factory.Name.ToLower() == BuildTable.ArmVehicleFactory )
            {
                if( random.Next( 1, 6 ) < 5 )
                {
                    unitdef = buildtable.UnitDefByName[ BuildTable.ArmL1Tank ] as IUnitDef;
                }
                else
                {
                    unitdef = buildtable.UnitDefByName[ BuildTable.ArmL1AntiAir ] as IUnitDef;
                }
            }
...
//            }
        ...
        {
        	return BuildTable.Tanks.Contains( name );
        }
        ...
        public void UnitAdded( int id, IUnitDef unitdef )
        {
            ...
            {
                if( !TankDefsById.ContainsKey( id ) )
                {
