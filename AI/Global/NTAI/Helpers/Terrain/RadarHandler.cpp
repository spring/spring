...
float3 CRadarHandler::NextSite(int builder, const UnitDef* unit, int MaxDist){
    int w = int(G->cb->GetMapWidth() / 8);
    int h = int(G->cb->GetMapHeight() / 8);
    int bx = int(G->cb->GetUnitPos(builder).x / 64.0f);
    int by = int(G->cb->GetUnitPos(builder).z / 64.0f);
