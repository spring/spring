	...
	if(valid){
		...
		if(construction == true){
			...
			if (targ != string("")){
				...
				if(udk != 0){
					if(G->UnitDefHelper->IsMobile(udk)){
						spacing =0;
					}
				}
				NLOG("calling CBuild2");
				bool r = G->Manufacturer->CBuild(targ,uid,spacing);
				NLOG("called CBuild2");
				return r;
			}else{
				return false;
			...
			if(targ == string("")){
				...
				switch (type) {
					case B_FACTORY: {
						spacing = atoi(G->Get_mod_tdf()->SGetValueDef("5", "AI\\factory_spacing").c_str());
						break;
					}
					case B_HUB: {
						spacing = atoi(G->Get_mod_tdf()->SGetValueDef("6", "AI\\factory_spacing").c_str());
						break;
					}
					case B_DEFENCE:{
						spacing = atoi(G->Get_mod_tdf()->SGetValueDef("4", "AI\\defence_spacing").c_str());
