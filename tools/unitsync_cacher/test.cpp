...
*/
  ...
  json.push_back(Pair("mapcount",mapCount));

  Object maps;
  for (int i = 0; i < mapCount; i++) {
    const string mapName = GetMapName(i);
	maps.push_back(Pair(toString(i),mapName));
    printf("    [map %3i]   %s\n", i, mapName.c_str());
  }
  json.push_back(Pair("mapIDs",maps));
  ...
  UnInit();
	...
	for (int i = 0; i < modCount; i++) {
		...
		m.push_back(Pair("luaoptions",GetOptions(modOptCount)));
		...
		mods.push_back(Pair(toString(i),m));
