...
BOOST_AUTO_TEST_CASE( UnitSync )
{
	...
	int i = GetMapCount() - 1, j = GetPrimaryModCount() - 1;
	while (i > 0 && !GetMapName(i)) --i;
	while (j > 0 && !GetPrimaryModName(j)) --j;

	const string map = GetMapName(i);
	const string mod = GetPrimaryModName(j);
	LOG("MAP = %s", map.c_str());
	LOG("MOD = %s", mod.c_str());

	LOG("GetSpringVersion() = %s", GetSpringVersion());
